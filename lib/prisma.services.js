  ...
  try {
    ...
    const filter = report.map((item) => {
      const clgf =
        ...
        findOneObject(facilities, item.cleaning[0].facilityId);
      const mlf =
        item.manualCleaning.length > 0 &&
        findOneObject(facilities, item.manualCleaning[0].facilityId);
      const mchf =
        item.machineCleaning.length > 0 &&
        findOneObject(facilities, item.machineCleaning[0].facilityId);
      ...
      return {
        ...
        cleaning:
          item.cleaning.length > 0
            ? {
                endoscopeId: item.endoscopeId,
                test: item.cleaning[0].test ? "Пройден" : "No",
        ...
        manualCleaning:
          item.manualCleaning.length > 0
            ? {
                endoscopeId: item.id,
                temperature: item.manualCleaning[0].temperature,
        ...
        machineCleaning:
          item.machineCleaning.length > 0
            ? {
                concentration: item.machineCleaning[0].solutionConcentration,
                endoscopeId: item.endoscopeId,
                processingMode: item.machineCleaning[0].processingMode,
                numberMdm: item.machineCleaning[0].numberMdm,
                start_time: formatDateToTimeLocal(
